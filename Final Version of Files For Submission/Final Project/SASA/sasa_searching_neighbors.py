...
for pdbid in cs_all['id'].unique()[0 :]:
	...
	reg = Ridge(alpha = 5, tol = 0.0001).fit(trainX_scaled, trainy)
	...
	## R2-score analysis
	score.append(r2_score(testy, predicted, multioutput = 'raw_values'))

	## Test model with training set
	predicted = reg.predict(trainX_scaled)
	score_itself.append(r2_score(trainy, predicted, multioutput = 'raw_values'))
